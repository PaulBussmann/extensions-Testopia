...
sub test_check_product{
	...
	# If the user does not have rights to check a product, this should die
		unless(Bugzilla->user->in_group('Testers') ){
			...
			my $db_obj = Bugzilla::Extension::Testopia::Test::Util::get_rep_by_field('products', 'id', $obj->product_id);
			# Check product by ID
...
sub test_set_description(){
	my $db_obj = Bugzilla::Extension::Testopia::Test::Util::get_rep_by_field(DB_TABLE, ID_FIELD, $obj->id);
...
sub test_create{
	...
	my $created_obj = Bugzilla::Extension::Testopia::Build->create($obj_hash);
