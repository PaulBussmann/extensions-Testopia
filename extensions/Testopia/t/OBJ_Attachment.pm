...
sub case_id_create{
	my $obj_hash = shift;
	my $created_obj = Bugzilla::Extension::Testopia::Attachment->create($obj_hash);
...
sub plan_id_create{
	...
	my $db_obj = Bugzilla::Extension::Testopia::Test::Util::get_rep('test_attachment_data',
										'attachment_id', $created_obj->{'attachment_id'} );
...
sub _link_case{
	...
	my $case = Bugzilla::Extension::Testopia::TestCase->create({'case_status_id' => 1,
													 'category_id'  => 1,
													 'priority_id'     => 1,
													 'author_id'     => $obj->id,
													 'plans'     => \@plans,
