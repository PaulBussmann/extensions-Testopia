...
sub check_bugs{
		my $creds =  Bugzilla::Extension::Testopia::Test::Constants->LOGIN_CREDENTIALS;
...
sub test_create{
	...
	# If the user does not have rights to create a Test Case, this should die
		unless(Bugzilla->user->can_see_bug(999)){
			...
			my $db_obj = new Bugzilla::Extension::Testopia::TestCase($created_obj->id);
...
sub test_update{
	...
	delete $obj->{'type'};
	$obj->update;
	my $db_obj = new Bugzilla::Extension::Testopia::TestCase($obj->id);
...
sub test_get_category_list{
    ...
    foreach my $id (@$cat_ids){
        push @categories, Bugzilla::Extension::Testopia::Category->new($id);
    }
...
sub test_add_to_run{
	...
	my $object = new Bugzilla::Extension::Testopia::TestCaseRun($db_obj->{'case_run_id'});
